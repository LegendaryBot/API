...
public class DiscordUserTest {
    ...
    public void testDiscordUser() {
        ...
        mainCharacterGuild.put(1234);
        mainCharacterGuild.put(4567);
        ...
        discordUser.setid(1111);
        discordUser.setJson(object.toString());

        JSONObject mainCharacter = DiscordUserHelper.getGuildMainCharacter(discordUser, 1234);
        ...
        mainCharacter = DiscordUserHelper.getGuildMainCharacter(discordUser, 4567);
        assertEquals(mainCharacter.length(), 3);
        assertEquals(mainCharacter.getString("region"), "us");
        assertEquals(mainCharacter.getString("realm"), "arthas");
        assertEquals(mainCharacter.getString("name"), "Kugruon");

        mainCharacter = DiscordUserHelper.getGuildMainCharacter(discordUser, 3);
        assertEquals(mainCharacter.length(), 0);

        DiscordUserHelper.setGuildMainCharacter(discordUser, 1234, "eu","moon-guard", "Kogorof");
        ...
        characterKugruon.getJSONArray("mainCharacterForGuild").forEach(value -> assertNotEquals(value, 1234));

        JSONObject characterKogorof = characterArray.getJSONObject(1);
        characterKogorof.getJSONArray("mainCharacterForGuild").forEach(value -> assertEquals(value, 1234));
    }
}
